...
uint64_t * manualptVlookup(uint64_t AD){ //TODO: measure cpu usage
	//USE: 0x1c000
	*((uint64_t*)(CBASE+0x130e0)) = AD|0x3;	
	FLUSH_TLB();
	return (uint64_t*)(CBASE+0x1c000);
}
uint64_t get_pml4V(int pml4){ //return value: integer content of pml4
			
	return ((uint64_t*)(CBASE+0x10000))[pml4];
}
...
void * KPALLOC(){ // THIS FUNCTION BREAKS IF CALLED TWICE
	...
	for(int i = 0; i<512; i++){
		AN = i;
		if(((uint64_t*)(CBASE+0x16000))[i] != 0x0){
			k = ((uint64_t*)(CBASE+0x16000))[i];
			((uint64_t*)(CBASE+0x16000))[i] = 0x0;
			break;
		}
	}
	if(k == 0x0){
	...
	if(AN > 500){
		if(!PA_CALLING){
			for(int i = 0; i<511; i++){
				if(((uint64_t*)(CBASE+0x16000))[i]==0x0) continue;
				((uint64_t*)(CBASE+0x16000))[i] = (uint64_t)KP_ALLOC3();
			}
		}
	}
	return (void*)k;
...
void * PL_MAP_START = (void*)(CBASE+0x15000);
...
void PL_SV(uint64_t index, uint8_t val){
	*(uint64_t*)LL_NV(index/64, PL_MAP_START) |= val << (index%64);
}
uint64_t PL_FN(){
	int i = 0;
...
void P_ALLOC3_SETUP(){
	...
	((uint64_t*)(CBASE+0x16000))[501]=CBASE+0x19000; // pde list plus pdpt list is two extra pages to start
	((uint64_t*)(CBASE+0x16000))[502]=CBASE+0x1A000;
	((uint64_t*)(CBASE+0x17000))[502]=CBASE+0x1B000;

	//fill up the rest of the list...
	((uint64_t*)(CBASE+0x16000))[511]=(uint64_t)KPALLOC();
	//remove bootstrap !!!
	((uint64_t*)(CBASE+0x10000))[0] = 0x0;
}
uint64_t V2P(void * V){
					...
					if(pdpt<511){
						...
						if(pml4<0){
							if((get_pml4V(pml4+1)&0x1) == 0){
								...
								uint64_t* M2 = KPALLOC();
								uint64_t* M3 = KPALLOC();
								((uint64_t*)(CBASE+0x10000))[pml4+1]=V2P(M)|0x27;
								M[0]=V2P(M2)|0x27;
								M2[0]=V2P(M3)|0x27;
					...
					if(pdpt<511){
						...
						if(pml4<511){
							if((get_pml4V(pml4+1)&0x1) == 0){
								...
								uint64_t* M2 = KPALLOC();
								uint64_t* M3 = KPALLOC();
								((uint64_t*)(CBASE+0x10000))[pml4+1]=V2P(M)|0x23;
								M[0]=V2P(M2)|0x23;
								M2[0]=V2P(M3)|0x23;
								M3[0]=(0x1000*ad)|0x1;
								PA_CALLING=0x0;
